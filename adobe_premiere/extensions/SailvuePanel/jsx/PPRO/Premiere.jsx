...
$._PPP_={
	...
	keepPanelLoaded : function () {
		app.setExtensionPersistent("com.adobe.SailvuePanel", 0); // 0, while testing (to enable rapid reload); 1 for "Never unload me, even when not visible."
	},
	...
	renameProjectItem : function () {
		...
		if (item) {
			item.name = item.name + ", updated by SailvuePanel.";
		} else {
	...
	createSequenceMarkers : function () {
		...
		if (activeSequence) {
			...
			newCommentMarker.name			= 'Marker created by SailvuePanel.';
			newCommentMarker.comments		= 'Here are some comments, inserted by SailvuePanel.';
			newCommentMarker.end			= (newCommentMarker.seconds + 5.0);

			var newWebMarker				= markers.createMarker(14.345);
			newWebMarker.name				= 'Web marker created by SailvuePanel.';
			newWebMarker.comments			= 'Here are some comments, inserted by SailvuePanel.';
	...
	mungeXMP : function () {
		...
		if (projectItem) {
			...
			if (ExternalObject.AdobeXMPScript !== undefined) {
				...
				xmp.setProperty(XMPConst.NS_DM, "scene", oldSceneVal + " Added by SailvuePanel sample!");
				xmp.setProperty(XMPConst.NS_DM, "creator", oldDMCreatorVal + " Added by SailvuePanel sample!");
				...
				if (numCreatorValuesPresent > 0) {
					...
					xmp.appendArrayItem(XMPConst.NS_DC,
										creatorProp,
										"SailvuePanel wrote the first value into NS_DC creator field.",
										null,
										XMPConst.ARRAY_IS_ORDERED);
				}
			...
			if (projectItem) {
				if (projectItem.type == ProjectItemType.CLIP || projectItem.type == ProjectItemType.FILE) {
					...
					if (markers) {
						...
						newMarker.name			= 'Marker created by SailvuePanel.';
						newMarker.comments		= 'Here are some comments, inserted by SailvuePanel.';
		...
		if (app.isDocumentOpen()) {
			...
			if (projectItem) {
				...
				if (ExternalObject.AdobeXMPScript !== undefined) { // safety-conscious!
					...
					var oldLogValue 				= "";
					var appendThis 					= "This log note inserted by SailvuePanel.";
					...
					xmp.setProperty(kPProPrivateProjectMetadataURI, newField, 	"SailvuePanel set this, using addPropertyToProjectMetadataSchema().");
					...
					if (foundYet) {
						$._PPP_.updateEventPanel("SailvuePanel successfully added a field to the project metadata schema, and set a value for it.");
					}
				}
			} else {
				$._PPP_.updateEventPanel("No project items found.");
	...
	onEncoderJobError : function (jobID, errorMessage) {
		...
		if (plugplugLibrary){
			...
			eventObj.type	= "com.adobe.csxs.events.SailvuePanelRenderEvent";
	...
	onProxyTranscodeJobComplete : function (jobID, outputFilePath) {
		...
		var nameToFind			= 'Proxies generated by SailvuePanel';
	...
	getPPPInsertionBin : function () {
		var nameToFind	= "Here's where SailvuePanel puts things.";
		var targetBin	= $._PPP_.searchForBinWithName(nameToFind);
				...
				if (newTrackItem) {
					...
					if (moComp) {
						...
						if (srcTextParam) {
							...
							srcTextParam.setValue("New value set by SailvuePanel!");
						}
					}
				}
			} else {
	...
	confirmPProHostVersion : function () {
		...
		if (version < 14.0) {
			$._PPP_.updateEventPanel("Note: SailvuePanel relies on features added in 14.0, but is currently running in " + version + ".");
		}
	},
		...
		*/
		...
		if (activeSequence) {
			...
			if (cloneAnyway) {
				...
				newSeq.name 		= newSeq.name + ", cloned by SailvuePanel.";
			}
		} else {
			$._PPP_.updateEventPanel("No active sequence.");
		}
	...
	autoReframeActiveSequence : function () {
		...
		if (seq) {
			if (seq.isDoneAnalyzingForVideoEffects()) {
				...
				var newName 		= seq.name + ", auto-reframed by SailvuePanel.";
